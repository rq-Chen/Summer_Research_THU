...
DFPATH = '..\new\ydy\Language\shantianfang\avrRef\';
EEGFILE = 'ydyStfData.mat';
isDirected = 1;
...
for jLoop = 1:1
    for iLoop = 2:NWINLEN % should be 1:NWINLEN if newly created
        ...
        tic;
        for K_CLUSTER = 2:5  % number of clusters
            ...
            for i = 1:clNum
                ...
                for icol = 1:size(currPhi, 2) - 1
                    for jcol = icol + 1:size(currPhi, 2)
                        clData(i, loopI) = PLI(currPhi(:, icol),....
                            currPhi(:, jcol), isDirected);
                        loopI = loopI + 1;
                    end
                end                
...
save([DFPATH EEGFILE], 'eegdata', 'anaSig');
...
DFPATH = '..\new\ydy\rest\avrRef\close\';
EEGFILE = 'ydyCloseData.mat';
isDirected = 1;
load([DFPATH EEGFILE], 'eegdata');
% The file should contain the numeric matrix (channels * timepoints) of the
% pre-processed EEG data, entitled eegdata; and the cell array (channels * 1)
...
DFPATH = '..\new\ydy\rest\avrRef\open\';
EEGFILE = 'ydyOpenData.mat';
isDirected = 1;
load([DFPATH EEGFILE], 'eegdata');
% The file should contain the numeric matrix (channels * timepoints) of the
% pre-processed EEG data, entitled eegdata; and the cell array (channels * 1)
...
DFPATH = '..\new\hc\shantianfang\avrRef\';
EEGFILE = 'hcStfData.mat';
isDirected = 1;
load([DFPATH EEGFILE], 'eegdata');
% The file should contain the numeric matrix (channels * timepoints) of the
% pre-processed EEG data, entitled eegdata; and the cell array (channels * 1)
